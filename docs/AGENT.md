...
- **Torch env: no torchaudio.** `torchaudio` is an orphan here (nothing requires it) and any CUDA build breaks `import transformers` via `audio_utils` (guard passes on presence, then the `.pyd` load crashes). Keep it UNINSTALLED; if reinstalled, it must version-match the CPU torch. Missing `langchain-opendataloader-pdf` (now pinned in pyproject) breaks `import app.main` — `pip install` it.
