...
| 9 | **Test coverage is minimal & environment-bound:** the single test loads local models on app construction and only checks `/api/health`. **Fixed:** `backend/tests/` is now a 14-test integration suite (real DB/models/APIs, no mocks) covering notebooks/files/messages CRUD, upload extension handling, end-to-end ingestion, `VectorRAG` retrieval, and both prompt endpoints. Run with `agent_env`; prompt tests skip if the LLM is down. | `backend/tests/test_app.py` (fixed) |
